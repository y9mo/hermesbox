...
for pass in 1 2; do
  ANSIBLE_LOCAL_TEMP="$test_dir/ansible" ansible-playbook \
    ...
    -e omp_builder_mistral_key=disposable-container-test \
...
docker exec --user omp-builder "$container" bash -lc \
  'test "$GH_TOKEN" = disposable-container-test && test "$RUNINFRA_GATEWAY_KEY" = disposable-container-test && test "$DEEPSEEK_API_KEY" = disposable-container-test && test "$MISTRAL_API_KEY" = disposable-container-test'
